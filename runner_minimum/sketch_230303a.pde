...
int judge_onbuilding(){
   ...
   for(int i=0;i<build.getsize();i++){
       ...
       if((player_x<=building_x+building_w/2)&&(player_x>=building_x-building_w/2)&&(player_y+player_r/2>=800-building_h))
       {
         p1.edity(800-building_h-player_r/2);
         p1.editv(0);
         return 1;
       }
      if((player_x<=building_x-building_w/2)&&(player_x>=building_x_last+building_w_last/2)&&(player_y+player_r/2>=800-building_h)&&(p1.getv()<0))
      {
        text("lose",100,200);
       return 2; 
      }
